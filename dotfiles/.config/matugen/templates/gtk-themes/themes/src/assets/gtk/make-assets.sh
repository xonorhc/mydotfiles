...
for theme in '' '-Blue' '-Green' '-Grey' '-Orange' '-Pink' '-Purple' '-Red' '-Teal' '-Yellow'; do
    for type in '' '-Darker' '-Oceanic' '-Palenight'; do
        ...
        if [[ "$type" != '' ]]; then
            ...
            sed -i "s/#82aaff/${theme_color_dark}/g" "assets${theme}${type}.svg"
            sed -i "s/#6182b8/${theme_color_light}/g" "assets${theme}${type}.svg"
            sed -i "s/#fafafa/${background_light}/g" "assets${theme}${type}.svg"
            sed -i "s/#0f111a/${background_dark}/g" "assets${theme}${type}.svg"
            sed -i "s/#090b10/${base_dark}/g" "assets${theme}${type}.svg"
            sed -i "s/#3b3f51/${surface_dark}/g" "assets${theme}${type}.svg"
        elif [[ "$theme" != '' ]]; then
            cp -rf "assets.svg" "assets${theme}.svg"
            sed -i "s/#82aaff/${theme_color_dark}/g" "assets${theme}.svg"
            sed -i "s/#6182b8/${theme_color_light}/g" "assets${theme}.svg"
        fi
    done
done
