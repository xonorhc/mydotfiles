...
return {
	{
		...
		priority = 1000,
		config = function()
			-- Função local para aplicar as cores geradas pelo Matugen
			local function apply_colors()
				...
				local guide_color = "{{colors.outline_variant.default.hex}}"
				vim.api.nvim_set_hl(0, "LineNr", { fg = guide_color })
			end

			apply_colors()

			-- Register a signal handler for SIGUSR1 (matugen updates)
			if not vim.g.matugen_signal_attached then
				local signal = vim.uv.new_signal()
				signal:start(
					"sigusr1",
					vim.schedule_wrap(function()
						package.loaded["plugins.matugen"] = nil
						local ok, plugin = pcall(require, "plugins.matugen")
						if ok and plugin then
							apply_colors()
						end
					end)
				)
				vim.g.matugen_signal_attached = true
			end
		end,
	},
}
